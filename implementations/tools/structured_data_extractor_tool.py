...
class StructuredDataExtractorTool(Tool):
    ...
    Может работать с большими файлами, используя RAG-подход внутри себя, если файл еще не проиндексирован
    или напрямую запрашивается извлечение из файла.
    """
    def __init__(self, text_extractor_tool: Tool, llm_for_parsing: AbstractLLM, vector_store: Optional[VectorStore], predefined_field_sets: Dict[str, str]):
        super().__init__(
            ...
            description=(
                ...
                "по заданному названию набора полей (например, 'кадастровый_объект'). "
                ...
                "Используй этот инструмент, когда пользователь просит извлечь КОНКРЕТНУЮ информацию из файла по набору полей."
            )
        )
    ...
    def _chunk_text(self, text: str, max_chunk_size: int = 1500, overlap_size: int = 200) -> List[str]:
        """
        Разбивает текст на чанки максимального размера с указанным перекрытием.
        """
        if not text:
            return []
        chunks = []
    ...
    async def execute(self, file_path: str, field_set_name: str, language: Optional[str] = None) -> str:
        ...
        extraction_instruction_prompt = (
            ...
            f"Если текста недостаточно для извлечения всех полей, укажи это, но все равно выведи то, что смог найти. "
            f"Список полей для извлечения:\n{fields_prompt}\n\n"
            f"Текст для анализа:\n"
        )
        
        # Порог для прямого анализа LLM (в символах, можно настроить)
        LLM_DIRECT_PARSE_THRESHOLD = 5000 
        
        final_extraction_content_for_llm = extracted_text

        if len(extracted_text) > LLM_DIRECT_PARSE_THRESHOLD:
            # Если текст большой, используем RAG-подход внутри инструмента
            ...
            logger.info(f"StructuredDataExtractor: Текст большой ({len(extracted_text)} символов). Разделяю на чанки и индексирую временно для RAG-извлечения.")
            chunks = self._chunk_text(extracted_text)
            
            # Для надежности, можно добавить уникальный ID файла.
            # Здесь мы добавляем чанки в ВЕКТОРНОЕ ХРАНИЛИЩЕ АГЕНТА,
            # чтобы они были доступны для последующих запросов через RAG агента.
            # Если StructuredDataExtractorTool вызывается, то он уже сам должен их найти.
            file_unique_id = f"file_{os.path.basename(file_path).replace('.', '_')}_{os.urandom(4).hex()}"
            metadatas = [
                {"source_file": file_path, "chunk_index": i, "extraction_id": file_unique_id, "type": "structured_extraction_source"} 
                for i in range(len(chunks))
            ]
            await self.vector_store.add_documents(chunks, metadatas=metadatas)
            logger.info(f"StructuredDataExtractor: Файл '{file_path}' проиндексирован с ID '{file_unique_id}' в основное векторное хранилище.")
            ...
            retrieved_chunks_raw = await self.vector_store.similarity_search(rag_query, k=5) # Извлекаем 5 самых релевантных чанков
            ...
            if not retrieved_chunks_raw:
                ...
                return f"Не удалось найти релевантную информацию в проиндексированном файле '{os.path.basename(file_path)}' для извлечения полей по набору '{field_set_name}'. Попробуйте задать более конкретный вопрос."
            
            # Формируем контент для LLM из извлеченных чанков, чтобы LLM их проанализировала
            final_extraction_content_for_llm = "\n---\n".join(retrieved_chunks_raw)
            logger.info(f"StructuredDataExtractor: Извлечено {len(retrieved_chunks_raw)} релевантных чанков для LLM-анализа.")

        # Вызываем LLM для извлечения данных из (возможно, укороченного или RAG-полученного) текста
        try:
            ...
            llm_system_prompt = (
                ...
                "Если текста недостаточно для извлечения всех полей, укажи это в начале ответа, "
                "но все равно выведи то, что смог найти."
            )
            ...
            else: # Если LLM ответила вызовом инструмента, это ошибка для этого инструмента
